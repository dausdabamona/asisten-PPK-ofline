"""
TUP List Page - Daftar semua transaksi Tambahan Uang Persediaan
"""
...
class TUPListPage(UPListPage):
    """Halaman daftar transaksi TUP (cloned dari UP dengan modifikasi)"""
    
    def __init__(self, parent=None):
        # Don't call parent __init__ yet, customize first
        super().__init__(parent)
        
        # Override mekanisme ke TUP
        self.mekanisme = 'TUP'
        
        # Customize UI
        self._customize_tup_ui()
    
    def _customize_tup_ui(self):
        """Customize UI untuk TUP"""
        # Find dan update title/header
        for widget in self.findChildren(type(self)):
            pass
    
    def load_data(self):
        """Override load_data untuk TUP"""
        status_map = {
            'Semua Status': None,
            'Draft': 'draft',
            'Aktif': 'aktif',
        ...
        for row, transaksi in enumerate(transaksi_list):
            ...
            kode_item.setForeground(QColor('#f39c12'))  # Orange for TUP
    ...
    def _update_summary(self):
        """Update summary statistics untuk TUP"""
        draft = self.pencairan_mgr.count_transaksi(mekanisme='TUP', status='draft')
        aktif = self.pencairan_mgr.count_transaksi(mekanisme='TUP', status='aktif')
        selesai = self.pencairan_mgr.count_transaksi(mekanisme='TUP', status='selesai')
